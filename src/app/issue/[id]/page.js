...
export default function IssueDetail() {
  ...
  if (!issue) return <div className="flex items-center justify-center min-h-screen text-gray-500">Issue not found</div>
  ...
  return (
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <button onClick={() => router.back()} className="flex items-center gap-1 text-sm text-gray-400 hover:text-indigo-500 mb-4">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card-gradient">
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                <span className="badge bg-charcoal-50 text-gray-300">{issue.category}</span>
              </div>
            ...
            <div className="card-gradient">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-semibold flex items-center gap-2"><Sparkles size={18} className="text-emerald-500" /> AI Insights</h2>
                <button onClick={handlePrediction} disabled={predicting} className="text-sm text-emerald-500 hover:text-emerald-400 font-medium">
                  {predicting ? 'Analyzing...' : prediction ? 'Refresh' : 'Predict Escalation'}
                </button>
              </div>
              {prediction ? (
                <div className={`p-4 rounded-lg ${prediction.willEscalate ? 'bg-red-500/10 border border-red-500/20' : 'bg-emerald-500/10 border border-emerald-500/20'}`}>
                  <div className="flex items-center gap-2 mb-2">
                    {prediction.willEscalate ? <AlertTriangle size={18} className="text-red-400" /> : <CheckCircle size={18} className="text-emerald-400" />}
                    <span className={`font-medium ${prediction.willEscalate ? 'text-red-300' : 'text-emerald-300'}`}>
                  ...
                  <p className="text-sm text-gray-300 mb-1"><strong>Reason:</strong> {prediction.reason}</p>
                  <p className="text-sm text-gray-300"><strong>Action:</strong> {prediction.recommendedAction}</p>
                </div>
              ) : (
                <p className="text-sm text-gray-500">Use AI to predict if this issue may escalate and needs urgent attention.</p>
              )}
            </div>
                ...
                {issue.statusHistory?.map((h, i) => (
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-gray-500 ml-2">{formatDate(h.at)}</span>
                    </div>
                  </div>
            ...
            <div className="card-gradient">
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto">
                ...
                {issue.comments?.slice().reverse().map((c) => (
                  <div key={c.id} className="flex gap-2 text-sm p-3 bg-charcoal-50 rounded-lg">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    {c.userPhoto && <img src={c.userPhoto} alt="" className="w-7 h-7 rounded-full shrink-0" />}
                    <div>
                      <span className="font-medium">{c.userName}</span>
                      <span className="text-xs text-gray-500 ml-2">{timeAgo(c.createdAt)}</span>
          ...
          <div className="space-y-6">
            ...
            <div className="card-gradient text-center">
              ...
              <div className="flex justify-center gap-6 mb-3">
                <button onClick={() => handleVote('up')} className="flex flex-col items-center text-gray-400 hover:text-green-600 transition-colors">
                  <ThumbsUp size={28} />
                  <span className="text-lg font-bold mt-1">{issue.upvotes || 0}</span>
                </button>
                <button onClick={() => handleVote('down')} className="flex flex-col items-center text-gray-400 hover:text-red-600 transition-colors">
                  <ThumbsDown size={28} />
                  <span className="text-lg font-bold mt-1">{issue.downvotes || 0}</span>
                </button>
              </div>
              <p className="text-xs text-gray-500">Vote to verify this issue</p>
            </div>
